...
from aioquic.h3.connection import H3Connection, FrameType, H3Stream, HeadersState, FrameUnexpected
from aioquic.h3.events import (
    DatagramReceived,
    DataReceived,
    H3Event,
...
### OVERWRITING EXISTING FUNCTIONS TO DEACTIVATE VALIDATIONS

class H3ConnectionInsecure(H3Connection):
    def __init__(self, quic, enable_webtransport = False) -> None:
    ...
    def _handle_request_or_push_frame(
        ...
        if frame_type == FrameType.DATA:
            ...
            # content-length needs checking even when there is no data (not anymore)
            #if stream_ended:
            #    self._check_content_length(stream)
    ...
    def http_event_received(self, event: H3Event) -> None:
        if isinstance(event, HeadersReceived) and event.stream_id not in self._handlers:
            ...
            for header, value in event.headers:
                if header == b"smuggling-id":
                    content = b"####REQ_ID_" + value + b"####" + content
                else:
                    content += b"####H_NAME####" + header + b"####H_VALUE####" + value
            ...
            if not content.startswith(b"####REQ_ID"):
                content = b"####REQ_ID_None" + content + b"####"
    ...
    def quic_event_received(self, event: QuicEvent) -> None:
        if isinstance(event, ProtocolNegotiated):
            if event.alpn_protocol in H3_ALPN:
                self._http = H3ConnectionInsecure(self._quic, enable_webtransport=True)
